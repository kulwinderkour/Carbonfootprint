...
export function AICoachMessage({ persona, totalKg, topCategory, previousTotalKg }: Props) {
  ...
  useEffect(() => {
    ...
    fn({ data: { persona, totalKg, topCategory, previousTotalKg: previousTotalKg ?? null } })
      .then((r) => {
        if (!cancelled) setText(r.message);
      })
      .catch((e: unknown) => {
        if (!cancelled) setErr(e instanceof Error ? e.message : "Coach unavailable");
      })
      .finally(() => {
  ...
  return (
    ...
    >
      ...
      {loading && (
        <p className="text-sm text-muted-foreground">Writing your personalized message…</p>
      )}
