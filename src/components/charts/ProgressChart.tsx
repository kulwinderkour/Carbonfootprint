...
export function ProgressChart({ data }: { data: HistoryPoint[] }) {
  if (data.length === 0) {
    return <p className="text-sm text-muted-foreground">No history yet — log your first footprint to see a trend line.</p>;
  }
  return (
    <figure aria-labelledby="progress-title">
      ...
      <div className="h-64 w-full" role="img" aria-label="Line chart showing total monthly carbon footprint over time. See list below.">
        <ResponsiveContainer>
          <LineChart data={data}>
      ...
      <ul className="mt-2 space-y-1 text-sm text-muted-foreground">
        {data.map((d) => (
          <li key={d.date} className="flex justify-between">
            <span>{d.date}</span><span className="tabular-nums">{formatKg(d.total)}</span>
          </li>
        ))}
      </ul>
    </figure>
