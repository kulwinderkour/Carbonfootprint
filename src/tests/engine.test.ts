...
import { calculateFootprint, calcEnergy, calcTransport, calcFood, calcWaste, calcWater, categoryPercentages } from "@/lib/calculator";
...
const empty: FootprintInput = {
  transport: {}, flightsPerMonth: 0, electricityUnitsKwh: 0, housing: "rented_flat",
  acHoursPerDay: 0, diet: "vegan", wasteLandfillKg: 0, wasteRecycledKg: 0, wasteCompostedKg: 0, waterLitersPerDay: 0,
};
...
describe("clamp & validation", () => {
  ...
  it("sanitizes a wild payload", () => {
    const out = sanitizeFootprintInput({ transport: { petrol_car: -99 }, electricityUnitsKwh: 1e9, diet: "bogus" as never } as never);
...
describe("emissionCalculator", () => {
  ...
  it("waste sum", () => {
    expect(calcWaste({ ...empty, wasteLandfillKg: 10, wasteRecycledKg: 10, wasteCompostedKg: 10 })).toBeCloseTo(6.5, 2);
  });
  it("water — 200 L/day", () => expect(calcWater({ ...empty, waterLitersPerDay: 200 })).toBeCloseTo(1.79, 1));
  it("breakdown sums to total", () => {
    const i: FootprintInput = { ...empty, transport: { petrol_car: 200 }, electricityUnitsKwh: 250, diet: "mixed", wasteLandfillKg: 5, waterLitersPerDay: 150 };
    const r = calculateFootprint(i);
    const sum = r.breakdown.transport + r.breakdown.energy + r.breakdown.food + r.breakdown.waste + r.breakdown.water;
    expect(r.totalKg).toBeCloseTo(sum, 2);
  });
  it("impact tier high when >500", () => {
    const r = calculateFootprint({ ...empty, transport: { petrol_car: 2000 }, electricityUnitsKwh: 400, diet: "heavy_meat" });
...
describe("recommendationEngine", () => {
  ...
  it("T2 (not E2) for hosteller with high energy", () => {
    const recs = getRecommendations({ ...empty, transport: { petrol_car: 300 }, electricityUnitsKwh: 400 }, "hosteller");
  ...
  it("returns at most 5", () => {
    const recs = getRecommendations({
      ...empty, transport: { petrol_car: 1000, scooter_petrol: 400 }, flightsPerMonth: 3,
      electricityUnitsKwh: 600, acHoursPerDay: 6, diet: "heavy_meat",
      wasteLandfillKg: 30, wasteRecycledKg: 1, wasteCompostedKg: 0, waterLitersPerDay: 300,
    }, "homeowner");
  ...
  it("sorted by priority × benefit (descending score)", () => {
    const recs = getRecommendations({
      ...empty, transport: { petrol_car: 500 }, flightsPerMonth: 3, diet: "heavy_meat",
    }, "student");
    for (let i = 0; i < recs.length - 1; i++) {
      expect(recs[i].priority * recs[i].estimatedBenefitKg).toBeGreaterThanOrEqual(recs[i + 1].priority * recs[i + 1].estimatedBenefitKg);
    }
  });
});
