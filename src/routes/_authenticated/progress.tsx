...
export const Route = createFileRoute("/_authenticated/progress")({
  head: () => ({
    meta: [
      { title: "Progress — Verdant" },
      { name: "description", content: "Your footprint trend and weekly goals." },
    ],
  }),
  component: Progress,
});
...
function Progress() {
  ...
  return (
    <Layout>
      ...
      {delta !== null && (
        <p
          className={`mb-6 rounded-xl border border-border p-4 text-sm ${delta < 0 ? "bg-primary/10 text-primary" : "bg-accent/30"}`}
        >
          {delta < 0
      ...
      <section aria-labelledby="g-h" className="mt-10 rounded-2xl border border-border bg-card p-6">
        <h2 id="g-h" className="mb-4 text-xl font-semibold">
          Your weekly goals
        </h2>
        {goals.data && goals.data.length > 0 ? (
            ...
            {(
              ...
              <li key={g.id} className="flex justify-between rounded-lg border border-border p-3">
                ...
                <span className="text-muted-foreground">
                  Week of {g.week_start} · {formatKg(g.target_reduction_kg)}
                </span>
              </li>
            ))}
          </ul>
          ...
          <p className="text-sm text-muted-foreground">
            No goals saved yet. Set some from the dashboard.
          </p>
        )}
      </section>
    </Layout>
