...
import type { CategoryBreakdown, FootprintEntryRow, FootprintInput, Persona } from "@/types/footprint";
import { sanitizeFootprintInput } from "@/lib/validation";
import { toast } from "sonner";

export const Route = createFileRoute("/_authenticated/dashboard")({
  head: () => ({ meta: [{ title: "Dashboard — Verdant" }, { name: "description", content: "Your latest carbon footprint and recommendations." }] }),
  component: Dashboard,
});
...
function Dashboard() {
  ...
  if (history.isLoading || profile.isLoading) {
    return <Layout><p className="text-muted-foreground">Loading your dashboard…</p></Layout>;
  }
    ...
    return (
      <Layout>
        <div className="rounded-2xl border border-border bg-card p-8 text-center">
          ...
          <p className="mt-2 text-muted-foreground">Log your first month to unlock recommendations and the AI coach.</p>
          <Button asChild className="mt-5"><Link to="/calculator">Start calculator</Link></Button>
        </div>
      </Layout>
    );
  }
  ...
  const diffVsIndia = Math.round(((latest.total_kg - INDIA_AVERAGE_MONTHLY_KG) / INDIA_AVERAGE_MONTHLY_KG) * 100);
    ...
    <Layout>
      <header className="mb-8 flex flex-wrap items-baseline justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">Latest entry from {new Date(latest.created_at).toLocaleDateString()}.</p>
        </div>
        <Button asChild variant="outline"><Link to="/calculator">Log a new month</Link></Button>
      </header>
      ...
      <section aria-label="Headline metric" className="grid gap-6 md:grid-cols-3">
        <div className="md:col-span-2 rounded-2xl bg-[image:var(--gradient-hero)] p-6 text-primary-foreground">
          ...
          <p className="mt-1 text-5xl font-bold tabular-nums">{latest.total_kg.toFixed(0)}<span className="ml-1 text-lg font-medium">kg</span></p>
          <p className="mt-2 text-sm text-primary-foreground/80">
            {diffVsIndia >= 0 ? `${diffVsIndia}% above` : `${Math.abs(diffVsIndia)}% below`} the India average ({INDIA_AVERAGE_MONTHLY_KG} kg/mo).
          </p>
          <p className="mt-3 text-sm">
            Biggest lever: <span className="font-semibold">{CATEGORY_META[result.topCategory].label}</span>
          </p>
        </div>
      ...
      <div className="mt-10 grid gap-8 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-border bg-gradient-to-br from-primary/10 to-accent/30 p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Tap the robot button in the bottom-right corner to chat with Verdant's AI coach.
            It already knows your latest footprint ({latest.total_kg.toFixed(0)} kg) and biggest lever
            ({CATEGORY_META[result.topCategory].label.toLowerCase()}) — ask for one habit, a swap, or a deeper explainer.
          </p>
          <p className="mt-4 text-xs text-muted-foreground">
            Previous month: {previous ? `${previous.total_kg.toFixed(0)} kg CO₂e` : "no prior reading yet"}.
          </p>
        </div>
      </div>

      <section aria-labelledby="recs-h" className="mt-12">
        <h2 id="recs-h" className="mb-4 text-2xl font-semibold tracking-tight">Top recommendations for you</h2>
        <RecommendationPanel recs={recs} />
      </section>

      <section aria-labelledby="goals-h" className="mt-12 rounded-2xl border border-border bg-card p-6">
        <h2 id="goals-h" className="mb-4 text-xl font-semibold tracking-tight">Suggested weekly goals</h2>
        {goals.length === 0 ? (
          <p className="text-sm text-muted-foreground">No goals to suggest — you're doing great.</p>
        ) : (
          <ul className="space-y-3">
            {goals.map((g, i) => (
              <li key={i} className="flex items-center justify-between gap-3 rounded-xl border border-border bg-background p-4">
                <div>
                  <p className="text-sm font-medium">{g.description}</p>
                  <p className="text-xs text-muted-foreground">Target this week: {formatKg(g.targetReductionKg)} CO₂e</p>
                </div>
                <Button size="sm" variant="outline" onClick={() => commit(i)}>Set goal</Button>
              </li>
            ))}
          </ul>
        )}
